...
export const DATA_SAVER_PRESETS = [
  ...
  { id: '2mb', label: '2 MB · Standard', mb: 2, maxPosts: 50, desc: 'Subway / Train (~50 posts)' },
...
export function getOfflineStash() {
  if (typeof window === 'undefined') {
    return { posts: [], sponsored: [], bookmarks: [], meta: null };
  }
  try {
    const raw = localStorage.getItem(STASH_KEY);
    return raw ? JSON.parse(raw) : { posts: [], sponsored: [], bookmarks: [], meta: null };
  } catch {
    return { posts: [], sponsored: [], bookmarks: [], meta: null };
  }
}
...
export function saveOfflineStash({ posts = [], sponsored = [], bookmarks = [], budgetMB = 2 }) {
...
export function clearOfflineStash() {
  if (typeof window === 'undefined') return;
  try {
    localStorage.removeItem(STASH_KEY);
    window.dispatchEvent(new CustomEvent('zerobar_stash_updated', { detail: null }));
  } catch (e) {
    console.warn('Could not clear offline stash:', e);
  }
}
  ...
  for (const action of outbox) {
    try {
      if (action.type === 'CREATE_POST') {
        ...
        const { post_id, reaction_type } = action.payload;
        if (!reaction_type) {
          await supabaseClient.from('post_reactions').delete().eq('user_id', user.id).eq('post_id', post_id);
        } else {
          // Upsert reaction
          await supabaseClient.from('post_reactions').delete().eq('user_id', user.id).eq('post_id', post_id);
          await supabaseClient.from('post_reactions').insert({
            user_id: user.id,
            post_id,
            reaction_type
          });
