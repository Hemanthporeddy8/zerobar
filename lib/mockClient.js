...
const DEFAULT_COMMENTS = [
  {
    ...
    user_id: 'user-1',
    post_id: 'post-2',
    content: 'Completely agree! Stashing the feed before flight takeoff is a game changer.',
  ...
  },
  {
    ...
    user_id: 'user-2',
    post_id: 'post-1',
    content: 'Frequency is noticeable during peak hours today. Good step.',
...
function getStore() {
  ...
  if (saved) {
    try {
      ...
      if (!parsed.comments) parsed.comments = [...DEFAULT_COMMENTS];
      return parsed;
    } catch {
      // Fallback
    }
...
class MockQueryBuilder {
  ...
  async execute() {
    ...
    if (this.tableName === 'posts') {
      ...
      const profiles = store.profiles || [];
      rows = rows.map((comm) => {
        const profile = profiles.find((p) => p.id === comm.user_id) || {
          username: 'Anonymous',
          avatar_emoji: '🧑'
        };
        return {
